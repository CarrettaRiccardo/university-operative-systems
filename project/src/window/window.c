...
message_t buildInfoResponseWindow(int to_pid, short state, int open_time);
...
int main(int argc, char **argv) {
    ...
    unsigned int last_open_time = time(NULL);   // Tempo ultima apertura lampadina
    ...
    while (1) {
        ...
        if (result == -1) {
            ...
            if (msg.type == DELETE_MSG_TYPE) {
                ...
                unsigned int work_time = open_time + (now - ((stato == 0) ? now : last_open_time));  //se è chiusa ritorno solo "open_time", altrimenti open_time+differenza da quanto accesa
