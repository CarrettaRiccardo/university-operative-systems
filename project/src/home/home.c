...
int switchDevice(int id, int label, int pos);
...
int main(int sargc, char **sargv) {
    ...
    while (run) {
        ...
        switch(command.type){
            case LIST_MSG_TYPE: listDevices(command.sender); break;
            case ADD_MSG_TYPE:{
            ...
            case DELETE_MSG_TYPE:{
                message_t ret = buildDeleteResponse(command.sender, delDevice(ret.vals[DELETE_VAL_DEST]));
                if (sendMessage(&ret) == -1)
                    perror("Error: sending message home "); 
            } break;
            case LINK_MSG_TYPE:{
                message_t ret = buildLinkResponse(command.sender, linkDevices(ret.vals[LINK_VAL_PID], ret.vals[LINK_VAL_DEST]));
                if (sendMessage(&ret) == -1)
                    perror("Error: sending message home "); 
            } break; 
            ...
            case INFO_MSG_TYPE:{
                message_t impose_command = buildInfoRequest( command.vals[INFO_VAL_DEST] );
